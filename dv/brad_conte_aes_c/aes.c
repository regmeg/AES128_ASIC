...
void AddRoundKey(uchar state[][4], uint w[])
{
  ...
  printf("Printing state just after AddRounkey \n");
  printstate(state);
  printf("Printing state round key used for AddRounkey \n");
  print_uint_array(w,4);
}
...
void SubBytes(uchar state[][4])
{
  ...
  printf("Printing state just after SubBytes \n");
  printstate(state);
}
...
void ShiftRows(uchar state[][4])
{
  ...
  printf("Printing state just after Shifrows \n");
  printstate(state);
}
...
void MixColumns(uchar state[][4])
{
  ...
  printf("Printing state just after MixColumns \n");
  printstate(state);
}
...
void rev_array(uchar arr_old[], uchar arr_new[], int length)
{
	...
	for (idx=0; idx < length; idx++)
		temp[idx] = arr_old[idx];
	
	for (idx=0; idx < length; idx++)
		arr_new[length - idx -1 ] = temp[idx];
}
...
void print_uchar_array(uchar data[], int itterator)
{
	int idx;
	for (idx=0; idx < itterator; idx++) printf("%02x",data[idx]);
  puts("");

}

void print_uint_array(uint data[], int itterator)
{
	int idx;
	for (idx=0; idx < itterator; idx++)
   printf("%08x",data[idx]);
   puts("");
...
uint KE_ROTWORD(uint temp)
{
	...
	new_temp[2] = temp >> 8;
	new_temp[3] = temp;
	//new_temp = ( ((temp) << 8) | ((temp) >> 24) );
	return ((new_temp[1] << 24) | (new_temp[2] << 16) | (new_temp[3] << 8) | new_temp[0]);
	}
...
{
	/*printf("Input key is \n ");
	print_uchar_array(key,keysize/8);*/
   ...
   for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
      temp = w[idx - 1];
      if ((idx % Nk) == 0)
				  //printf("temp = %08x, rotword(temp) = %08x, SubWord(KE_ROTWORD(temp)) = %08x, Rcon[%d]= %08x \n", temp, KE_ROTWORD(temp), SubWord(KE_ROTWORD(temp)), ((idx)/Nk)-1, Rcon[((idx)/Nk)-1]);
         temp = SubWord(KE_ROTWORD(temp)) ^ Rcon[((idx)/Nk)-1];
         //printf("temp after mod = %08x \n", temp);}
      else if (Nk > 6 && (idx % Nk) == 4)
         temp = SubWord(temp);
      w[idx] = w[idx-Nk] ^ temp;
      //printf("w[idx - 1]:[%d] = %08x, w[idx-Nk]:[%d] = %08x, w[idx]:[%d]= %08x \n",idx - 1,w[idx - 1],idx-Nk, w[idx-Nk], idx, w[idx]);
   
		}
	//printf("key Schedule is \n ");
	//print_uint_array(w,4*(Nr+1));
}
/********************
** Functions for dpi
********************/
//Function to copy data to state matrix
void copy_data_to_state(uchar in[])
{
	state[0][0] = in[0];
  state[1][0] = in[1];
  state[2][0] = in[2];
...
// Function to copy data out of state matrix
void get_data_from_state(uchar out[])
{
	out[0] = state[0][0];
  out[1] = state[1][0];
  out[2] = state[2][0];
  ...
  out[15] = state[3][3];
	...
	//Function to test single block_wide_add_roun_key
	void aes_encrypt_block_wide_add_rnd_key_test(uchar in[], uchar out[], uchar key_in[])
{
	int idx;
	uint key[4];
	
	//Print key
	//printf("The 8bit key array is ");
	//print_uchar_array(key_in,16);
  /*for (idx=0; idx < 16; idx++)
      printf("%02x ",key_in[idx]);
   puts("");
   
	//Cast 8bit key array into 32bit array
	printf("key_in[0] is %04x\n",key_in[0]);
	printf("key_in[0] << 8 is %04x\n",key_in[0] << 8);
	printf("(key_in[0] << 8) | key_in[0] is %04x\n",(key_in[0] << 8) | key_in[0]);*/
	
	key[0] = ((key_in[0] << 24) | (key_in[1] << 16) | (key_in[2] << 8) | key_in[3]);
	key[1] = ((key_in[4] << 24) | (key_in[5] << 16) | (key_in[6] << 8) | key_in[7]);
	key[2] = ((key_in[8] << 24) | (key_in[9] << 16) | (key_in[10] << 8) | key_in[11]);
	key[3] = ((key_in[12] << 24) | (key_in[13] << 16) | (key_in[14] << 8) | key_in[15]);
	
	
	//printf("Printing the round key\n");
	//print_rnd_key(key);
	//Copy data to the state matrix
	copy_data_to_state(in);
	//printf("Printing state just after data copied in\n");
  //printstate(state);
  
  //Perform add round key
  AddRoundKey(state,key);
  
  //Copy data out of state matrix
  //printf("Printing state before data is output\n");
  //printstate(state);
	get_data_from_state(out);
}

void aes_encrypt_mix_columns_test(uchar in[], uchar out[])
{
	rev_array(in, in,16);
	...
	rev_array(out, out,16);
}

//Run the sbox test
void aes_encrypt_subbytes_test(uchar in[], uchar out[])
{
	copy_data_to_state(in);
...
//Pack sbox values into one serials signal
void get_sboxvalues(uchar out[])
{
	int i,j;
	for (i=0; i < 16; ++i) {
		for (j=0; j < 16; ++j) {
			out[i*16+j] = aes_sbox[i][j];
			//printf("out[%d] = %08x, aes_sbox[%d][%d] = %08x",i*16+j, out[i*16+j],i,j,aes_sbox[i][j]);
			//puts("");
		}
	}
}
// Get key_expansion_values
void get_key_expnasion(uchar key[], int keysize, uchar keyschedule_val[], int key_expansion_indx)
{
	int Nr;
   ...
   default: return;
	...
	uint key_expnasion[4*(Nr+1)];
	
	// The dpi reverses input vectors MSB abd and makes them LSB of the output array, reverse have to be done for the consistency.
	rev_array(key,key,keysize/8);
	
	KeyExpansion(key,key_expnasion,keysize);
	
	//SystemVerilog does not work with uints, hence data has to be converted into uchars
	keyschedule_val[0] = key_expnasion[key_expansion_indx] >> 24;
	keyschedule_val[1] = key_expnasion[key_expansion_indx] >> 16;
	keyschedule_val[2] = key_expnasion[key_expansion_indx] >> 8;
	keyschedule_val[3] = key_expnasion[key_expansion_indx];
	

	/*printf("keyschedule_val[0] is %02x\n",keyschedule_val[0]);
	printf("keyschedule_val[1] is %02x\n",keyschedule_val[1]);
	printf("keyschedule_val[2] is %02x\n",keyschedule_val[2]);
	printf("keyschedule_val[3] is %02x\n",keyschedule_val[3]);
	printf("key_expnasion[%d] is %08x",key_expansion_indx,key_expnasion[key_expansion_indx]);*/

	//Reverse data again for consistency.
	rev_array(keyschedule_val,keyschedule_val,4);

}

void aes_encrypt_shiftrows_test(uchar in[], uchar out[]) {
	
	//printf("data_in_before reverse \n ");
	//print_uchar_array(in,16);
	
	rev_array(in, in,16);
	
	//printf("data_in_after reverse \n ");
	//print_uchar_array(in,16);
	...
	//printf("data_out_before reverse \n ");
	//print_uchar_array(out,16);

	rev_array(out, out,16);
	
	//printf("data_out_after reverse \n ");
	//print_uchar_array(out,16);
}
...
void aes_encrypt(uchar in[], uchar out[], uint key[], int keysize)
{
  ...
  printstate(state);
   ...
   printf("ROUND 0 Starting 0 cycle Addroundkey \n");
   AddRoundKey(state,&key[0]);
   printf("ROUND 1 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[4]);
   printf("ROUND 2 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[8]);
   printf("ROUND 3 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[12]);
   printf("ROUND 4 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[16]);
   printf("ROUND 5 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[20]);
   printf("ROUND 6 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[24]);
   
  //printf("Printing state at some random stage");
  //printstate(state);
   printf("ROUND 7 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[28]);
   printf("ROUND 8 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[32]);
   printf("ROUND 9 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
   ...
   if (keysize != 128) {
      ...
      if (keysize != 192) {
         ...
         SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[48]);
		  ...
		  printf("ROUND 10 Subbytes, ShiftRows, MixColumns, AddRoundKey \n");
      SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[40]);
   }

   // Copy the state to the output array
   printf("Printing state after encprytion done \n");
   printstate(state);
...
void aes_complete_encrypt_test(uchar data_in[], uchar data_out[], uchar key_in[], int keysize)
{
 	int Nr;
   ...
   default: return;
	...
	rev_array(data_in, data_in,16);
	rev_array(key_in, key_in,keysize/8);

	KeyExpansion(key_in,key_schedule,keysize);
  aes_encrypt(data_in,data_out,key_schedule,keysize);
   
  rev_array(data_out, data_out,16);
